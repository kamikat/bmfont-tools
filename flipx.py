...
def flipx(line, bits):
    text = line.encode('hex')
    step = bits / 4
    stack = []
    for i in xrange(0, len(text), step):
        stack.append(text[i:i+step])
    return ''.join(reversed(stack)).decode('hex')

if __name__ == '__main__':

    import argparse

    parser = argparse.ArgumentParser(description='Flip X coordinate of font data')
    ...
    parser.add_argument('-o', '--outfile',
            default='flipx.bin',
            help='Output result to (default flipx.bin)')
